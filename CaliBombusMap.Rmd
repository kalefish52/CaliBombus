---
title: "CaliBombus Map"
output: html_document
---

#Load Packages
```{r}
library(dplyr)
library(ggplot2)
library(tidyr)
library(sf)
library(tmap)


```
#Import data 
```{r}
#Import ecoregion shape file from EPA/USGS Level III 
ecoregionshape<-st_read("./ca_eco_l3/ca_eco_l3.shp") %>% st_transform("+proj=longlat +datum=WGS84")
head(ecoregionshape)
#Check coordinate system 
st_crs(ecoregionshape)
ecoregionshape$US_L3NAME

##Recode variable names so they are shorter 
ecoregionshape$US_L3NAME<-recode(ecoregionshape$US_L3NAME, 'Central Basin and Range' = "CentralBasinRange")
ecoregionshape$US_L3NAME<-recode(ecoregionshape$US_L3NAME, 'Coast Range' = "CoastRange")
ecoregionshape$US_L3NAME<-recode(ecoregionshape$US_L3NAME, 'Mojave Basin and Range' = "Mojave")
ecoregionshape$US_L3NAME<-recode(ecoregionshape$US_L3NAME, 'Central California Foothills and Coastal Mountains' = "CentralCaliforniaFoothills")
ecoregionshape$US_L3NAME<-recode(ecoregionshape$US_L3NAME, 'Central California Valley' = "CentralValley")
ecoregionshape$US_L3NAME<-recode(ecoregionshape$US_L3NAME, 'Klamath Mountains/California High North Coast Range' = "Klamath")
ecoregionshape$US_L3NAME<-recode(ecoregionshape$US_L3NAME, 'Southern California/Northern Baja Coast' = "Southern California")
ecoregionshape$US_L3NAME<-recode(ecoregionshape$US_L3NAME, 'Eastern Cascades Slopes and Foothills' = "Eastern Cascades")

#Import site information 

site<-read.csv("./spatialinfofinal.csv") %>% select(Site_Grouping, lon, lat) 
site
#Have to put in as long/lat
site.sf <- st_as_sf(site, coords = c("lon", "lat"), crs = 4326) %>% st_transform("+proj=longlat +datum=WGS84")
site.sf


#Map with Labels 
tm_shape(ecoregionshape) + tm_polygons(col="US_L3NAME", border.col="white", palette="PuBuGn", title="Ecoregion")+ tm_legend(outside = TRUE)+tm_shape(site.sf) + tm_dots(size=0.2, col="black")+tm_text("Site_Grouping", just = "left", ymod = 0.5, size = 0.8) 

#Map without labels
tm_shape(ecoregionshape) + tm_polygons(col="US_L3NAME", border.col="white", palette="PuBuGn", title="Ecoregion")+tm_shape(site.sf) + tm_dots(size=0.2, col="black")+
 tm_layout(frame=FALSE, legend.text.size=0.4, legend.width=1)+tm_legend(legend.position=c("left", "top"), outside=TRUE)

```

## NIFA MAP 
```{r}
#Import new NIFA sites 
NIFAsites<-read.csv("./NIFASites.csv") %>% mutate(sampled="2020") %>% select(Site, lon, lat, Ecoregion, sampled) 
NIFAsites

sitecali<-read.csv("./spatialinfofinal.csv") %>% select(Site_Grouping, lon, lat, Ecoregion.1) 

#Combine NIFA with calibombus sites 
calisites<-sitecali %>% select(Site_Grouping, lon, lat, Ecoregion.1) %>% rename(Site=Site_Grouping) %>% rename(Ecoregion=Ecoregion.1) %>% mutate(sampled="2019")
calisites

#Bind both dataframes 
Sites_NIFA<-rbind(NIFAsites, calisites)
Sites_NIFA

#Have to put in as long/lat
site.sf <- st_as_sf(Sites_NIFA, coords = c("lon", "lat"), crs = 4326) %>% st_transform("+proj=longlat +datum=WGS84")


tm_shape(ecoregionshape) + 
  tm_polygons(col="US_L3NAME", border.col="white", palette="PuBuGn", title="Ecoregion")+
  tm_shape(site.sf) + 
  tm_dots(size=0.2, col="sampled", palette=c(yes='black', no='red'), title="Sampling Status")+
  tm_layout(frame=FALSE, legend.text.size=0.6, legend.width=1)+tm_legend(legend.position=c("left", "top"), outside=TRUE)






```
