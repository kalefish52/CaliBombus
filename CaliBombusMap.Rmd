...
#Subset to all collections after 1983
Fullpre83<-FullData %>% filter(year<="1983")
print(paste("We have", nrow(Fullpre83), "datapoints in or before 1983"))

#Subset to all collections after 1983
Fullpost83<-FullData %>% filter(year>"1983")
print(paste("We have", nrow(Fullpost83), "datapoints after 1983"))
...
Pre83<-Fullpre83 %>% select(BBNA.code, species, sex, year, longitude, latitude) %>% na.omit()
print(paste("We removed", (nrow(Fullpre83)-nrow(Pre83)), "datapoints that had NA coordinates"))

#Convert lat/long data to epsg 5070 to match ecoregion projection in rgdal package
COORDSpre83<-SpatialPoints(cbind(Pre83$longitude, Pre83$latitude), proj4string=CRS("+proj=longlat"))
coordspre83<-spTransform(COORDSpre83, CRS("+init=epsg:5070"))

#Transform to data frame and add site information 
convertpre83<-data.frame(coordspre83) 
convertp83comb<-cbind(convertpre83, Pre83) %>% 
  rename(x="coords.x1") %>% 
  rename(y="coords.x2")

#Convert to sf object to match with ecoregion 
site.sf.pre83 <- st_as_sf(convertp83comb, coords=c("x", "y"), crs=5070) 
site.sf.pre83

#Overlay gps points with ecoregion polygons to see which sites are in which ecoregions
Pre83_ecoregions<-point.in.poly(site.sf.pre83, ecoregions) %>% as.data.frame()
nrow(Pre83_ecoregions)
