...
# How many records do we have for each ecoregion?
EcoHist<-SpeciesEco %>% group_by(Ecoregion) %>% mutate(count=1) %>% mutate(EcoRecords=sum(count)) 

TotalRecords<-EcoHist%>% select(Ecoregion, EcoRecords) %>% unique()
TotalRecords

ggplot(TotalRecords, aes(x=Ecoregion, y=EcoRecords)) + geom_bar(stat="identity", position=position_dodge()) +ggtitle("Number of Historical Records in Each Ecoregion")+ theme(axis.text.x  = element_text(angle=90))

```

## Separate Historical Records by Year via Ecoregion 
```{r}
