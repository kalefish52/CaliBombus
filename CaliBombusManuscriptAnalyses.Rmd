...
group.colors <- c(Klamath = "#6B8E23", CoastalSage = "#6A5ACD", SierraNevada="#DAA520", CoastRange = "#C71585", Cascades = "#008080", CentralBasin="#B8860B")

NMDSvosplot<-ggplot() + geom_point(data=data.scores, aes(x=NMDS1, y=NMDS2, colour=Ecoregion), size=2)+ 
scale_fill_manual(values=group.colors)+geom_text_repel(data=species.scores, aes(x=NMDS1, y=NMDS2, label=species), position=position_jitter(width=1,height=1)) +scale_x_continuous(limits = c(-3.5, 3.5))
NMDSvosplot
