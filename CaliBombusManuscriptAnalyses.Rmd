...
NMDSplot<-ggplot() + 
  ...
  theme_bw()+scale_y_continuous(limits=c(-2,1.5))
NMDSplot
...
out<-iNEXT(NextEco, q=c(1), datatype="abundance", endpoint=1000)
...
commatrix<-TotalBreakdown %>% select(!c(Date_comb, Ecoregion.1, TotalAbundance, SpeciesRichness,  Ecoregion, coords.x1, coords.x2, X, X.1)) %>% unique()
...
head(TotalBreakdown)
dfeco<-TotalBreakdown %>% select(!c(Date_comb, TotalAbundance, SpeciesRichness,  Ecoregion, coords.x1, coords.x2, X, X.1)) %>% unique()
