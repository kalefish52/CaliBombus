...
SiteNum %>% filter(Verified.Bee.species.ID=="vosnesenskii")%>% 
...
cols<-c("Cascades"="aquamarine4", "SierraNevada"="darkgoldenrod", "CentralBasin"="darkgoldenrod4", 
        "Klamath"="olivedrab", "CoastRange"="plum4", "CoastalSage"="darkmagenta")
NMDSvosplot<-ggplot() + geom_point(data=data.scores, aes(x=NMDS1, y=NMDS2, colour=Ecoregion), size=2)+ 
scale_fill_manual(values=group.colors)+geom_text_repel(data=species.scores, aes(x=NMDS1, y=NMDS2, label=species), position=position_jitter(width=1,height=1)) +scale_x_continuous(limits = c(-3.5, 3.5))+theme_bw()+
  scale_colour_manual(values=cols)+scale_y_continuous(limits=c(-2,1.5))
NMDSvosplot


NMDSvosno<-ggplot() + geom_point(data=data.scoresvos, aes(x=NMDS1, y=NMDS2, colour=Ecoregion), size=2)+ geom_text_repel(data=species.scoresvos, aes(x=NMDS1, y=NMDS2, label=species))+scale_x_continuous(limits = c(-3.5, 3.5))+theme_bw()+scale_colour_manual(values=cols)+theme_bw()+scale_y_continuous(limits=c(-2,1.5))
