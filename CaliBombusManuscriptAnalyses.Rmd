...
## Diversity, Richness Metrics by Ecoregion and by Site using Vegan 
#Table 2 
```{r, echo=FALSE}
# Shannon's Diversity for Ecoregion level and Site Level 
##Site Level 
Shannonmatrix<-AllSpp %>% select(!c(Date_comb, SpeciesRichness, TotalAbundance, Ecoregion.1, Site_Grouping, coords.x1, coords.x2, Ecoregion, X))
...
#Table 2 Info 
Table2<-AllSiteRich %>% select(Site_Grouping, Ecoregion.1, SpeciesRichness, TotalAbundance, SiteShannon) 
Table2 %>% arrange(Ecoregion.1)


print(paste("avg richness per site is", mean(AllSiteRich$SpeciesRichness), "with a standard error of", (sd(AllSiteRich$SpeciesRichness)/sqrt(17))))



#Change Shannon to Integer 
Table2$SiteShannon=as.numeric(Table2$SiteShannon)

#Summarize average/sd richness, abundance, shannon - filter ecoregions with only one site
T2Sums<-Table2 %>% 
  group_by(Ecoregion.1) %>% 
  mutate(meanrich=mean(SpeciesRichness)) %>% 
  mutate(serich=(sd(SpeciesRichness)/sqrt(n()))) %>% 
  mutate(meanabund=mean(TotalAbundance)) %>% 
  mutate(seabund=(sd(TotalAbundance)/sqrt(n()))) %>% 
  mutate(MeanShan=mean(SiteShannon)) %>% 
  mutate(seshan=(sd(SiteShannon)/sqrt(n()))) %>% 
  select(Ecoregion.1, meanrich, serich, meanabund, seabund, MeanShan, seshan) %>% unique()
T2Sums
write.csv(T2Sums, "./Tables/T2Sums.csv")
```
...
cols<-c("Cascades"="aquamarine4", 
        "Sierra Nevada"="darkgoldenrod", 
        "Central Basin"="darkgoldenrod4", 
        "Klamath"="olivedrab", 
        "Coast Range"="plum4", 
...
NMDSplot<-ggplot() + 
  ...
  theme_bw()+scale_y_continuous(limits=c(-2,1))
NMDSplot


ggsave("./Figures/NMDS.png", width=7, height=6, dpi=600)
