...
TotalBreakdown<-dataFinal %>% filter(Site_Grouping != "misc") %>% select(!sex.caste) %>% unique() 
...
```{r}
H=diversity(SpeciesAcc, index="shannon")
H
...
ggplot() + geom_point(data=data.scores, aes(x=NMDS1, y=NMDS2, colour=Site), size=2)+ geom_text(data=species.scores, aes(x=NMDS1, y=NMDS2, label=species))
