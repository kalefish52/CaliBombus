...
head(dataTIME)
dataFinal<-dataTIME %>% filter(Site_Grouping != "misc") %>% filter(!Date_comb %in% c("7-May-19","3-Jun-19","6-Jul-19")) %>% select(Date_comb, Site_Grouping, Ecoregion, Verified.Bee.species.ID, sex.caste, SpeciesAbundance, SpeciesRichness, TotalAbundance, Latitude, Longitude, Elevation_ft) 
...
## Get dataframe with spatial info to make map 
```{r}
head(dataFinal)
spatialinfo<-(dataFinal) %>% select(Date_comb, Site_Grouping, Ecoregion, Latitude, Longitude, Elevation_ft) %>% unique()
write.csv(spatialinfo, "spatialinfo.csv")

## Changed site coordinates so that no sites were in areas we don't have sampling permits 
...
ggplot() + geom_point(data=data.scores, aes(x=NMDS1, y=NMDS2, colour=Ecoregion), size=2)+ geom_text(data=species.scores, aes(x=NMDS1, y=NMDS2, label=species), position=position_jitter(width=3, height=3))+scale_x_continuous(limits = c(-3.5, 3.5))
ggsave("NMDSVos.png")
...
## Plot it using ggplot
ggplot() + geom_point(data=data.scoresvos, aes(x=NMDS1, y=NMDS2, colour=Ecoregion), size=2)+ geom_text(size=4, data=species.scoresvos, aes(x=NMDS1, y=NMDS2, label=species), position=position_jitter(width=3, height=3))+scale_x_continuous(limits = c(-3, 3))
ggsave("NMDSnoVos.png")
## Anosim - are communities different by ecoregion
...
SN<-ggiNEXT(out, type=1) + theme_bw(base_size=10)+guides(linetype=FALSE)+ theme(legend.position = "bottom",
 ...
 text=element_text(size=10), plot.title=element_text(size=10), legend.key.size = unit(0.2, "cm"),
  legend.key.width = unit(0.2,"cm")) + ggtitle("Sierra Nevada")
...
CW<-ggiNEXT(out, type=1) + theme_bw(base_size=10)+guides(linetype=FALSE)+ theme(legend.position = "bottom",
 legend.title=element_blank(),
 text=element_text(size=10), plot.title=element_text(size=10),legend.key.size = unit(0.2, "cm"),
  legend.key.width = unit(0.2,"cm")) + ggtitle("Central Western")
...
NW<-ggiNEXT(out, type=1) + theme_bw(base_size=10)+guides(linetype=FALSE)+ theme(legend.position = "bottom",
 legend.title=element_blank(),
 text=element_text(size=10), plot.title=element_text(size=10), legend.key.size = unit(0.2, "cm"),
  legend.key.width = unit(0.2,"cm")) + ggtitle("Northwest")
...
CR<-ggiNEXT(out, type=1) + theme_bw(base_size=10)+guides(linetype=FALSE)+ theme(legend.position = "bottom",
 legend.title=element_blank(),
 text=element_text(size=10), plot.title=element_text(size=10), legend.key.size = unit(0.2, "cm"),
  legend.key.width = unit(0.2,"cm")) + ggtitle("Cascade Range")
...
SNE<-ggiNEXT(out, type=1) + theme_bw(base_size=10)+guides(linetype=FALSE)+ theme(legend.position = "bottom",
 legend.title=element_blank(),
 text=element_text(size=10), plot.title=element_text(size=10), legend.key.size = unit(.2, "cm"),
  legend.key.width = unit(.2,"cm")) + ggtitle("East of Sierra Nevada") 
SNE


final<-multiplot(SN, CW, NW, CR, SNE, cols=3) 



```

## Multiplot function from Rcookbook 
```{r, include=FALSE}
